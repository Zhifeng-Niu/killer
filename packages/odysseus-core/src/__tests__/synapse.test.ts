...
import { ColumnRole, type SynapseMessage } from '../synapse/types.js';
...
describe('SynapseProtocol', () => {
  ...
  const createColumnId = (id: string, type: ColumnRole = ColumnRole.Prime) => ({
    id,
    type,
    instance: 0,
  });
  ...
  describe('registerCell', () => {
    it('应注册新 Cell', () => {
      const cellId = createColumnId('cell-1');
      protocol.registerColumn(cellId);

      const cells = protocol.getAllColumns();
  ...
  describe('unregisterCell', () => {
    it('应注销 Cell', () => {
      ...
      protocol.unregisterColumn(cellId);
  ...
  describe('send', () => {
    it('应发送消息', () => {
      const senderId = createColumnId('sender');
      const receiverId = createColumnId('receiver');

      protocol.registerColumn(senderId);
      protocol.registerColumn(receiverId);
  ...
  describe('broadcast', () => {
    it('应广播消息给订阅者', () => {
      const cell1 = createColumnId('cell-1');
      const cell2 = createColumnId('cell-2');

      protocol.registerColumn(cell1);
      protocol.registerColumn(cell2);
  ...
  describe('fission', () => {
    it('应分裂 Cell', () => {
      const parentId = createColumnId('parent');
      protocol.registerColumn(parentId);
  ...
  describe('fusion', () => {
    it('应融合多个 Cell', () => {
      ...
      protocol.registerColumn(cell1, { capabilities: ['a'] });
      protocol.registerColumn(cell2, { capabilities: ['b'] });
  ...
  describe('listCapability', () => {
    it('应发布能力到市场', () => {
      const providerId = createColumnId('provider');
      protocol.registerColumn(providerId);
  ...
  describe('executeTransaction', () => {
    it('应执行交易', () => {
      const providerId = createColumnId('provider');
      const buyerId = createColumnId('buyer');

      protocol.registerColumn(providerId);
      protocol.registerColumn(buyerId);
  ...
  describe('discoverCells', () => {
    it('应发现所有 Cell', () => {
      protocol.registerColumn(createColumnId('cell-1', ColumnRole.Prime));
      protocol.registerColumn(createColumnId('cell-2', ColumnRole.Researcher));
    ...
    it('应按类型筛选 Cell', () => {
      protocol.registerColumn(createColumnId('prime-1', ColumnRole.Prime));
      protocol.registerColumn(createColumnId('researcher-1', ColumnRole.Researcher));

      const primeCells = protocol.discoverCells(ColumnRole.Prime);

      expect(primeCells.length).toBe(1);
      expect(primeCells[0].type).toBe(ColumnRole.Prime);
    });
  });
  ...
  describe('heartbeat', () => {
    it('应更新心跳时间', () => {
      ...
      const status = protocol.getColumnStatus(cellId);
      expect(status?.lastHeartbeat).toBeGreaterThanOrEqual(before);
    });

    it('未注册 Cell 心跳不应崩溃', () => {
      const cellId = createColumnId('ghost');
  ...
  describe('send edge cases', () => {
    it('不存在的发送者应抛出错误', () => {
      ...
      }).toThrow('Sender column ghost-sender not found');
    });
  });
  ...
  describe('broadcast edge cases', () => {
    it('不存在的发送者应抛出错误', () => {
      ...
      }).toThrow('Sender column ghost not found');
    });

    it('订阅者应收到广播消息', () => {
      const sender = createColumnId('sender');
      const receiver = createColumnId('receiver');
      protocol.registerColumn(sender);
      protocol.registerColumn(receiver);
  ...
  describe('fission edge cases', () => {
    it('不存在的父 Cell 应抛出错误', () => {
      expect(() => protocol.fission('ghost')).toThrow('Parent column ghost not found');
    });

    it('子 Cell 应继承父类型和配置', () => {
      const parentId = createColumnId('parent', ColumnRole.Researcher);
      protocol.registerColumn(parentId, { capabilities: ['search'], maxLoad: 15 });
      ...
      expect(child!.id.type).toBe(ColumnRole.Researcher);
  ...
  describe('fusion edge cases', () => {
    it('有不存在的 Cell 应抛出错误', () => {
      ...
      expect(() => protocol.fusion(['cell-1', 'ghost'])).toThrow('One or more columns not found');
    });

    it('融合后应合并能力并去重', () => {
      const c1 = createColumnId('cell-1');
      const c2 = createColumnId('cell-2');
      protocol.registerColumn(c1, { capabilities: ['a', 'b'] });
      protocol.registerColumn(c2, { capabilities: ['b', 'c'] });
  ...
  describe('getColumnStatus', () => {
    ...
    it('未注册 Cell 应返回 undefined', () => {
      const status = protocol.getColumnStatus(createColumnId('ghost'));
  ...
  describe('getTransactions', () => {
    it('应返回所有交易记录', () => {
      const provider = createColumnId('provider');
      const buyer = createColumnId('buyer');
      protocol.registerColumn(provider);
      protocol.registerColumn(buyer);
  ...
  describe('receive edge cases', () => {
    it('未注册 Cell 应返回空数组', () => {
      const messages = protocol.receive(createColumnId('ghost'));
  ...
  describe('getTopology', () => {
    it('无连接时应返回空边列表', () => {
      protocol.registerColumn(createColumnId('cell-1'));
      protocol.registerColumn(createColumnId('cell-2'));
  ...
  describe('clear', () => {
    it('应重置所有状态', () => {
      ...
      expect(protocol.getAllColumns()).toHaveLength(0);
  ...
  describe('unregisterCell cleanup', () => {
    it('应清理连接和订阅', () => {
      ...
      protocol.unregisterColumn(cell1);
