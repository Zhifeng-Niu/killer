...
import type { PluginManager, OdysseusPlugin } from '../plugins/index.js';
...
export async function loadPlugins(deps: PluginLifecycleDeps): Promise<void> {
  // 从项目目录和用户目录加载
  const pluginDirs = [
    path.join(process.cwd(), '.odysseus', 'plugins'),
    path.join(os.homedir(), '.odysseus', 'plugins'),
  ];
  ...
  for (const tool of pluginTools) {
    deps.tools.register({
      ...
      execute: tool.execute as (params: unknown) => Promise<import('@odysseus/core').ToolResult>,
    });
...
export async function registerPlugin(
  ...
  if (latestTool) {
    deps.tools.register({
      ...
      execute: latestTool.execute as (params: unknown) => Promise<import('@odysseus/core').ToolResult>,
    });
  }
}
