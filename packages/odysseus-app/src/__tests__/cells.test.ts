/**
 * ColumnManager Tests - Cell Lifecycle Management
 */

import { describe, it, expect } from 'vitest';
import { ColumnManager } from '../orchestrator/cells.js';
import { SynapseProtocol, ColumnRole, type ColumnId } from '@odysseus/core';

describe('ColumnManager', () => {
  function createManager(): { manager: ColumnManager; synapse: SynapseProtocol } {
    const synapse = new SynapseProtocol();
    const manager = new ColumnManager(synapse);
  ...
  describe('registerPrimeCell', () => {
    it('should register the prime cell', () => {
      ...
      expect(cells[0].id.type).toBe(ColumnRole.Prime);
    });
  ...
  describe('spawnCell', () => {
    it('should spawn a researcher cell', () => {
      ...
      expect(cellId!.type).toBe(ColumnRole.Researcher);
    ...
    it('should spawn an artisan cell', () => {
      ...
      expect(cellId).not.toBeNull();
      expect(cellId!.type).toBe(ColumnRole.Artisan);
    });
    ...
    it('should spawn a negotiator cell', () => {
      ...
      expect(cellId).not.toBeNull();
      expect(cellId!.type).toBe(ColumnRole.Negotiator);
    });
    ...
    it('should spawn an evolver cell', () => {
      ...
      expect(cellId).not.toBeNull();
      expect(cellId!.type).toBe(ColumnRole.Evolver);
    });
  ...
  describe('getColumnStatus', () => {
    it('should report status of registered cells', () => {
      ...
      const status = manager.getColumnStatus();
    ...
    it('should return empty array when no cells', () => {
      const { manager } = createManager();
      expect(manager.getColumnStatus()).toEqual([]);
    });
  });
  ...
  describe('critic and explorer cells', () => {
    it('should spawn a critic cell', () => {
      ...
      expect(cellId).not.toBeNull();
      expect(cellId!.type).toBe(ColumnRole.Critic);
    ...
    it('should spawn an explorer cell', () => {
      ...
      expect(cellId).not.toBeNull();
      expect(cellId!.type).toBe(ColumnRole.Explorer);
    ...
    it('should spawn both critic and explorer for Cerebellum collaboration', () => {
      ...
      expect(critic!.type).toBe(ColumnRole.Critic);
      expect(explorer!.type).toBe(ColumnRole.Explorer);
