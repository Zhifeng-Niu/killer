...
import { SynapseProtocol, ColumnRole, MockLLMProvider } from '@odysseus/core';
import { TaskDelegate } from '../orchestrator/task-delegate.js';
import type { ColumnId } from '@odysseus/core';
...
describe('TaskDelegate', () => {
  ...
  function createDelegate(mockResponse?: string): {
    ...
    synapse.registerColumn(primeColumnId, { name: 'prime', capabilities: ['all'], maxLoad: 10 });
  ...
  it('should handle LLM errors gracefully', async () => {
    ...
    const delegate = new TaskDelegate(synapse, failingLlm, primeColumnId);
  ...
  it('should parse artisan cell type from LLM response', async () => {
    ...
    const artisanTasks = result.subtasks.filter(st => st.cellType === ColumnRole.Artisan);
  ...
  it('should parse negotiator cell type from LLM response', async () => {
    ...
    expect(result.subtasks[0].cellType).toBe(ColumnRole.Negotiator);
  });
  ...
  it('should parse evolver cell type from LLM response', async () => {
    ...
    expect(result.subtasks[0].cellType).toBe(ColumnRole.Evolver);
  });
  ...
  it('should parse multiple subtasks from LLM response', async () => {
    ...
    expect(result.subtasks.map(st => st.cellType)).toEqual(
      [ColumnRole.Researcher, ColumnRole.Artisan, ColumnRole.Evolver],
    );
  });
  ...
  it('should assign cell IDs to subtasks', async () => {
    ...
    const assigned = result.subtasks.filter(st => st.assignedColumnId);
    expect(assigned.length).toBeGreaterThanOrEqual(1);
    for (const st of assigned) {
      expect(st.assignedColumnId).toBeDefined();
      expect(st.assignedColumnId!.type).toBe(ColumnRole.Researcher);
    }
  });
  ...
  it('should handle LLM fail during synthesize with fallback concatenation', async () => {
    ...
    const synapse = new SynapseProtocol();
    synapse.registerColumn(primeColumnId, { name: 'prime', capabilities: ['all'], maxLoad: 10 });
    const delegate = new TaskDelegate(synapse, partialFailLlm, primeColumnId);
