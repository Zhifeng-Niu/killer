...
describe('Session Persistence Integration', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'odysseus-session-test-'));
  });
  ...
  it('should persist and restore hippocampus memories', async () => {
    ...
    const agent1 = new OdysseusAgent(createTestAgentConfig(tempDir));
    await agent1.boot();
    ...
    const agent2 = new OdysseusAgent(createTestAgentConfig(tempDir));
    await agent2.boot();
  ...
  it('should persist cell state through sessions', async () => {
    const agent = new OdysseusAgent(createTestAgentConfig(tempDir));
    await agent.boot();
