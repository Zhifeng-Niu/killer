 ...
 *
 * 验证当用户只设了 API key 而没设 ODYSSEUS_LLM_PROVIDER 时，
 * 系统能自动推断正确的 provider。
 */
...
describe('Provider Auto-Detection', () => {
  ...
  it('should prefer explicit ODYSSEUS_LLM_PROVIDER over auto-detect', async () => {
    process.env.DEEPSEEK_API_KEY = 'sk-deepseek';
    process.env.ODYSSEUS_LLM_PROVIDER = 'glm';
  ...
  it('should use ODYSSEUS_API_KEY as generic key with default anthropic provider', async () => {
    process.env.ODYSSEUS_API_KEY = 'sk-generic';
    // 没有 provider 特定 key → 没有 auto-detect → 回到默认 anthropic
    // 但 ODYSSEUS_API_KEY 被设了
