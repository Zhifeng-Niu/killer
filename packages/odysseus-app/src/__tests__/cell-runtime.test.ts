 ...
 * ColumnRuntime Tests
 *
 * 测试 Cell 独立执行引擎和 ColumnRuntimePool
 */

import { describe, it, expect, beforeEach } from 'vitest';
import { SynapseProtocol, ColumnRole, type ColumnId, type LLMProvider, type LLMCompletion } from '@odysseus/core';
import { MockLLMProvider } from '@odysseus/core';
import { ColumnRuntime, ColumnRuntimePool } from '../orchestrator/cell-runtime.js';
...
describe('ColumnRuntime', () => {
  ...
  beforeEach(() => {
    ...
    cellId = { id: 'test-researcher', type: ColumnRole.Researcher, instance: 0 };
    synapse.registerColumn(cellId, {
  ...
  describe('Lifecycle', () => {
    it('should start and stop', () => {
      const runtime = new ColumnRuntime(cellId, synapse, llm, {}, (msg) => logs.push(msg));

      expect(runtime.isRunning()).toBe(false);
      expect(runtime.getColumnId()).toBe(cellId);
    ...
    it('should not double-start', () => {
      const runtime = new ColumnRuntime(cellId, synapse, llm);
      runtime.start();
      runtime.start(); // no-op
      expect(runtime.isRunning()).toBe(true);
      runtime.stop();
  ...
  describe('Direct Execution', () => {
    ...
    it('should handle LLM errors gracefully', async () => {
      ...
      const runtime = new ColumnRuntime(cellId, synapse, failingLlm);
    ...
    it('should respect timeout config', async () => {
      ...
      const runtime = new ColumnRuntime(cellId, synapse, hangingLlm, { timeoutMs: 100 });
  ...
  describe('Message-Driven Execution', () => {
    it('should handle incoming Synapse messages', async () => {
      ...
      synapse.registerColumn(primeColumnId, { name: 'prime', capabilities: [], maxLoad: 10 });

      const runtime = new ColumnRuntime(cellId, synapse, llm, {}, (msg) => logs.push(msg));
      runtime.start();

      // Send a request message to the cell
      synapse.send(primeColumnId.id, cellId.id, {
        id: 'msg-1',
        timestamp: Date.now(),
        type: 'request',
...
describe('ColumnRuntimePool', () => {
  ...
  describe('Pool Management', () => {
    it('should spawn and track cells', () => {
      const pool = new ColumnRuntimePool(synapse, llm, {}, (msg) => logs.push(msg));

      const r1 = pool.spawn(ColumnRole.Researcher);
      const r2 = pool.spawn(ColumnRole.Artisan);
    ...
    it('should retire individual cells', () => {
      const pool = new ColumnRuntimePool(synapse, llm);

      const runtime = pool.spawn(ColumnRole.Researcher);
      const id = runtime.getColumnId();
    ...
    it('should stop all cells', () => {
      ...
      pool.spawn(ColumnRole.Researcher);
      pool.spawn(ColumnRole.Artisan);
      pool.spawn(ColumnRole.Negotiator);
    ...
    it('should get a cell by id', () => {
      const pool = new ColumnRuntimePool(synapse, llm);
      const runtime = pool.spawn(ColumnRole.Evolver);

      const found = pool.get(runtime.getColumnId().id);
  ...
  describe('Capabilities', () => {
    it('should assign correct capabilities per cell type', () => {
      ...
      const cells = synapse.getAllColumns();
      const researcher = cells.find((c) => c.id.type === ColumnRole.Researcher);
      const artisan = cells.find((c) => c.id.type === ColumnRole.Artisan);
    ...
    it('should spawn critic and explorer with correct capabilities', () => {
      const pool = new ColumnRuntimePool(synapse, llm, {}, (msg) => logs.push(msg));
      pool.spawn(ColumnRole.Critic);
      pool.spawn(ColumnRole.Explorer);

      const cells = synapse.getAllColumns();
      const critic = cells.find((c) => c.id.type === ColumnRole.Critic);
      const explorer = cells.find((c) => c.id.type === ColumnRole.Explorer);
