...
async function handleKeyPaste(
  ...
  if (testResult.ok) {
    ...
    console.log(`  ✓ 连接成功！运行 odysseus 即可。`);
    return;
  }
  ...
  if (confidence === 'high') {
    ...
    await saveConfig(providerName, key, model, protocol);
    console.log(`  ✓ 已保存为 ${provider.description}。运行 odysseus 即可。`);
    return;
  }
  ...
  if (proceed.trim().toLowerCase() === 'y') {
    await saveConfig(providerName, key, model, protocol);
    console.log('  ✓ 已保存。运行 odysseus 即可。');
  } else {
    console.log('  已取消。运行 odysseus --init 重试。');
  }
}
...
export async function runInitWizard(): Promise<void> {
  ...
  try {
    console.log('');
    console.log('  ┌─────────────────────────────────────────────┐');
    console.log('  │  Odysseus Agent — 粘贴 API Key 即可开始        │');
    ...
    if (existingConfig) {
      ...
      if (keepExisting.trim().toLowerCase() !== 'n') {
        console.log('  ✓ 配置不变。运行 odysseus 即可启动。');
        return;
      }
      console.log('');
    }
    ...
    if (selected.name === 'mock') {
      ...
      console.log('  ✓ 体验模式。运行 odysseus 即可。');
      return;
    }
    ...
    if (!finalKey) {
      console.log('  Key 不能为空。运行 odysseus --init 重试。');
      return;
    }
...
function detectAvailableKeys(): Array<{ envKey: string; key: string }> {
  ...
  const odysseusKey = process.env.ODYSSEUS_API_KEY;
  if (odysseusKey && odysseusKey.trim() && !found.some(f => f.key === odysseusKey.trim())) {
    found.push({ envKey: 'ODYSSEUS_API_KEY', key: odysseusKey.trim() });
  }
  return found;
}
...
function detectExistingConfig(): { provider: string; apiKey: string; source: string } | null {
  ...
  const envProvider = process.env.ODYSSEUS_LLM_PROVIDER;
  if (envProvider) {
    const envKey = PROVIDER_OPTIONS.find(p => p.name === envProvider)?.envKey || 'ODYSSEUS_API_KEY';
    return {
      provider: envProvider,
      apiKey: process.env[envKey] || process.env.ODYSSEUS_API_KEY || '',
      source: '环境变量',
    };
  }
...
export async function saveConfig(
  ...
  const lines: string[] = [
    '# Odysseus Agent — auto-generated by init wizard',
    `ODYSSEUS_LLM_PROVIDER=${provider}`,
  ];

  if (apiKey) {
    const envKey = PROVIDER_OPTIONS.find(p => p.name === provider)?.envKey || 'ODYSSEUS_API_KEY';
    lines.push(`${envKey}=${apiKey}`);
  }

  if (model) {
    lines.push(`ODYSSEUS_MODEL=${model}`);
  }

  if (protocol && protocol !== 'openai') {
    lines.push(`ODYSSEUS_PROTOCOL=${protocol}`);
  }
  ...
  // 写入 ~/.odysseus/.env（全局）
  const odysseusDir = path.join(os.homedir(), '.odysseus');
  if (!fs.existsSync(odysseusDir)) {
    fs.mkdirSync(odysseusDir, { recursive: true });
  }
  fs.writeFileSync(path.join(odysseusDir, '.env'), envContent, 'utf-8');

  // 备份到 ~/.odysseus/config.json（不含明文 key）
