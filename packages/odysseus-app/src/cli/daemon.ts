 ...
 * Daemon Mode - 后台常驻模式
 *
 * 让 Odysseus Agent 在后台持续运行，用户可随时连接。
 * PID 文件: ~/.odysseus/daemon.pid
 * 日志文件: ~/.odysseus/daemon.log
 *
 * 用法:
 *   node odysseus.mjs --daemon          # 后台启动
 *   node odysseus.mjs --daemon --stop   # 停止后台进程
 *   node odysseus.mjs --daemon --status # 查看状态
 */
...
const ODYSSEUS_DIR = join(homedir(), '.odysseus');
const PID_FILE = join(ODYSSEUS_DIR, 'daemon.pid');
const LOG_FILE = join(ODYSSEUS_DIR, 'daemon.log');

function ensureKillerDir(): void {
  if (!existsSync(ODYSSEUS_DIR)) {
    mkdirSync(ODYSSEUS_DIR, { recursive: true });
  }
}
...
export function startDaemon(port: number): void {
  ...
  if (existingPid !== null && isProcessAlive(existingPid)) {
    ...
    console.log(`  Stop: node odysseus.mjs --daemon --stop`);
    process.exit(0);
  }
  ...
  console.log(`  Log: ${LOG_FILE}`);
  console.log(`  Stop: node odysseus.mjs --daemon --stop`);
  console.log(`  Status: node odysseus.mjs --daemon --status`);
  process.exit(0);
}
