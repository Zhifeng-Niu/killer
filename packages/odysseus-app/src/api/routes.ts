...
import { ValidationError, Cerebellum } from '@odysseus/core';
 ...
 * 注册所有 API 路由到服务器
 */
export function registerRoutes(server: APIServer, agent: OdysseusAgent): void {
  // === Health & Status ===
 ...
 */
function handleWSCommand(conn: WSConnection, data: { command: string; args?: Record<string, unknown> }, agent: OdysseusAgent): void {
  ...
  switch (command) {
    ...
    case 'skills':
      ...
      break;
    case 'cells':
      reply({ cells: agent.getColumnStatus() });
      break;
    case 'metrics': {
      const metrics = MetricsCollector.getInstance();
