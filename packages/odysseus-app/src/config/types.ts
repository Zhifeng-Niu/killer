 ...
 *
 * 支持 .odysseus/ 目录配置 + 环境变量 + 命令行参数
 *
 * 配置优先级（高到低）：
 * 1. CLI 参数
 * 2. 环境变量
 * 3. 项目级 .odysseus/config.json
 * 4. 用户级 ~/.odysseus/config.json
 * 5. 默认值
 */
...
/**
 * 配置文件结构（.odysseus/config.json）
 */
interface ConfigFile {
  llm?: Partial<OdysseusConfig['llm']>;
  agent?: Partial<OdysseusConfig['agent']>;
  memory?: Partial<OdysseusConfig['memory']>;
  sensory?: Partial<OdysseusConfig['sensory']>;
  prefrontal?: Partial<OdysseusConfig['prefrontal']>;
  logging?: Partial<OdysseusConfig['logging']>;
}

/**
 * 默认配置
 */
const DEFAULT_CONFIG: OdysseusConfig = {
  llm: {
    provider: 'anthropic',
    apiKey: '',
    temperature: 0.7,
 ...
 * 用于自动检测：当用户只设了 API key 而没设 ODYSSEUS_LLM_PROVIDER 时推断 provider
 */
...
function detectProviderFromEnvKeys(): { provider: string; apiKey: string } | null {
  // 优先 ODYSSEUS_API_KEY（通用 key，但需要 ODYSSEUS_LLM_PROVIDER 配合）
 ...
 *
 * 自动检测逻辑：
 * 1. 用户显式设了 ODYSSEUS_LLM_PROVIDER → 使用它 + 查找对应 key
 * 2. 用户只设了某个 provider 的 API key → 自动推断 provider + 使用该 key
 * 3. 什么都没设 → 返回空，后续走 mock/demo 模式
 */
function readEnvLLMConfig(): Partial<OdysseusConfig['llm']> {
  const config: Partial<OdysseusConfig['llm']> = {};

  const explicitProvider = process.env.ODYSSEUS_LLM_PROVIDER;

  if (explicitProvider) {
    // 用户显式指定了 provider
    config.provider = explicitProvider;
    const apiKey = process.env.ODYSSEUS_API_KEY || findApiKeyForProvider(explicitProvider);
    ...
    if (detected) {
      ...
      config.apiKey = process.env.ODYSSEUS_API_KEY || detected.apiKey;
    }
  }

  const model = process.env.ODYSSEUS_MODEL;
  if (model) config.model = model;

  const baseUrl = process.env.ODYSSEUS_BASE_URL;
  if (baseUrl) config.baseUrl = baseUrl;

  const protocol = process.env.ODYSSEUS_PROTOCOL;
 ...
 * 从环境变量读取 Agent 配置
 */
function readEnvAgentConfig(): Partial<OdysseusConfig['agent']> {
  const config: Partial<OdysseusConfig['agent']> = {};

  if (process.env.ODYSSEUS_DEBUG === 'true') config.debugLogging = true;
  if (process.env.ODYSSEUS_EVOLUTION === 'false') config.evolutionEnabled = false;
 ...
 * 从环境变量读取日志配置
 */
function readEnvLoggingConfig(): Partial<OdysseusConfig['logging']> {
  const config: Partial<OdysseusConfig['logging']> = {};

  const level = process.env.ODYSSEUS_LOG_LEVEL;
 ...
 */
export function loadConfig(cliOverrides?: DeepPartial<OdysseusConfig>): OdysseusConfig {
  // 1. 用户级配置
  const userConfigPath = path.join(os.homedir(), '.odysseus', 'config.json');
  const userConfig = readConfigFile(userConfigPath);

  // 2. 项目级配置
  const projectConfigPath = path.join(process.cwd(), '.odysseus', 'config.json');
  ...
  const config = merged as unknown as OdysseusConfig;
 ...
 */
function validateConfig(config: OdysseusConfig): void {
 ...
 * 初始化 .odysseus/ 目录结构
 */
export function initOdysseusDir(targetDir?: string): string {
  const dir = targetDir ?? path.join(process.cwd(), '.odysseus');
