...
export class PluginManager {
   ...
   * 从目录加载插件
   *
   * 搜索 .odysseus/plugins/ 和 node_modules/@killer/plugin-*
   */
   ...
   * 注册内联插件（编程方式）
   */
  async register(plugin: OdysseusPlugin): Promise<void> {
   ...
   * 从路径加载单个插件
   */
  private async loadPlugin(pluginPath: string): Promise<OdysseusPlugin | null> {
    // 尝试加载 JS 文件
    ...
    try {
      ...
      const plugin: OdysseusPlugin = module.default ?? module;
