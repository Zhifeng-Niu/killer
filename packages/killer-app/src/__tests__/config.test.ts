...
describe('Configuration System', () => {
  ...
  beforeEach(() => {
    ...
    for (const key of ['KILLER_LLM_PROVIDER', 'KILLER_API_KEY', 'KILLER_MODEL', 'KILLER_DEBUG', 'KILLER_LOG_LEVEL', 'ANTHROPIC_API_KEY']) {
  ...
  it('should return default config when nothing is set', () => {
    // Clear env
    delete process.env.KILLER_LLM_PROVIDER;
    delete process.env.KILLER_API_KEY;
    delete process.env.KILLER_MODEL;
    delete process.env.KILLER_DEBUG;
    delete process.env.KILLER_LOG_LEVEL;
  ...
  it('should read LLM config from environment', () => {
    process.env.KILLER_LLM_PROVIDER = 'openai';
    process.env.KILLER_API_KEY = 'test-key-123';
    process.env.KILLER_MODEL = 'gpt-4';
  ...
  it('should init .killer directory', () => {
    const tmpDir = path.join(os.tmpdir(), `.killer-test-${Date.now()}`);
    try {
      const dir = initKillerDir(tmpDir);
