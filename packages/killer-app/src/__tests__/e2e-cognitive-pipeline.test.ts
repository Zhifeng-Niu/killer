 ...
 *
 * 使用真实的 KillerAgent + MockLLMProvider 作为集成点。
 */
...
describe('E2E Cognitive Pipeline', () => {
  ...
  describe('session persistence and restore', () => {
    it('should save and restore cognitive state via hippocampus/persona export', async () => {
      ...
      // 创建新 agent 并恢复
      const restoredAgent = new KillerAgent(createTestConfig());
  ...
  describe('full cognitive relay (Samantha persistence)', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'killer-relay-test-'));
    });
    ...
    it('should relay complete cognitive state across agent lifecycle', async () => {
      ...
      const agent1 = new KillerAgent(createRelayConfig());
      await agent1.boot();
      ...
      const agent2 = new KillerAgent(createRelayConfig());
      await agent2.boot();
