 ...
 *
 * REPL-style interactive input loop for the Killer Agent CLI
 */
...
/** CLI 历史文件路径 */
const HISTORY_FILE = path.join(os.homedir(), '.killer', 'cli_history');
...
interface CLICommand {
  name: string;
  description: string;
  handler: (args: string, agent: KillerAgent) => Promise<void> | void;
}
...
const CLI_COMMANDS: CLICommand[] = [
  ...
  {
    name: 'spawn',
    description: 'Spawn a new cell (e.g., /spawn researcher)',
  ...
  },
  {
    ...
    handler: async (_args, agent) => {
      ...
      if (plugins.length === 0) {
        console.log('\n🔌 No plugins loaded.');
        console.log('   Add plugins to .killer/plugins/ and restart.');
      } else {
        console.log('\n🔌 Loaded Plugins:');
  ...
  {
    name: 'init',
    description: 'Initialize .killer/ directory or run setup wizard (/init setup)',
    handler: async (args, _agent) => {
      if (args.trim() === 'setup') {
        ...
        const { initKillerDir } = await import('../config/types.js');
        const dir = initKillerDir();
        console.log(`\n  Initialized .killer/ directory at: ${dir}`);
  ...
  },
  {
    ...
    handler: async (args, _agent) => {
      ...
      try {
        ...
        // Exit with code 42 → killer.mjs will auto-restart
        process.exit(42);
      } catch (err) {
 ...
 * 处理命令
 */
async function handleCommand(input: string, agent: KillerAgent): Promise<boolean> {
 ...
 * 启动交互式 readline 循环
 */
export function startReadlineLoop(agent: KillerAgent): readline.Interface {
  // 加载历史记录
 ...
 */
function generateFarewell(agent: KillerAgent): string {
  const parts: string[] = [];
