...
export function loadEnvFiles(dirs?: string[]): number {
  const searchDirs = dirs ?? [
    process.cwd(),
    // 向上查找 monorepo 根目录（包含 killer.mjs 或 package.json 的目录）
    findProjectRoot(),
    path.join(os.homedir(), '.killer'),
  ];
...
/**
 * 从当前目录向上查找项目根目录
 * 识别标志：存在 killer.mjs 或包含 packages/ 目录的 package.json
 */
function findProjectRoot(): string {
  let dir = process.cwd();
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, 'killer.mjs'))) return dir;
