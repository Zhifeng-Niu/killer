...
import { loadConfig, initKillerDir, type KillerConfig } from './config/index.js';
 ...
 * 显示启动横幅
 */
function showBanner(config: KillerConfig): void {
  const B = '\x1b[1m';
  const R = '\x1b[0m';
  const CYAN = '\x1b[36m';
  const DIM = '\x1b[2m';
...
function showHelp(): void {
  ...
  console.log(`
Killer Agent - AI Agent Framework
...
Environment Variables:
  KILLER_LLM_PROVIDER      LLM provider [default: anthropic]
...
Examples:
  node killer.mjs                     # Start with .env config
  node killer.mjs --demo              # Try instantly (no API key needed)
  node killer.mjs --init              # Interactive setup wizard
  pnpm start                          # Start via pnpm
  KILLER_LLM_PROVIDER=deepseek DEEPSEEK_API_KEY=sk-... node killer.mjs
`);
}
 ...
 * 创建 Agent 配置
 */
function createAgentConfig(config: KillerConfig, fresh: boolean): AgentConfig {
...
async function main(): Promise<void> {
  ...
  // 初始化 .killer/ 目录（失败不阻止启动）
  try {
    initKillerDir();
  } catch (err) {
    console.warn(`Warning: Could not create .killer directory: ${err instanceof Error ? err.message : String(err)}`);
  }

  // 加载配置（CLI > env > project .killer/config.json > ~/.killer/config.json > defaults）
  ...
  const agent = new KillerAgent(agentConfig);
  ...
  if (config.agent.debugLogging) {
    ...
    console.log(`[DEBUG] Config: .killer/config.json loaded`);
    console.log('');
  }
  ...
  const isCliMode = !api && !process.env.KILLER_DEBUG;
  ...
  try {
    ...
    console.error('  试试: node killer.mjs --init');
    process.exit(1);
  }
  ...
  if (api) {
    const apiToken = process.env.KILLER_API_TOKEN || undefined;
