...
- **Plugin System** — Dynamic loading from `.killer/plugins/`
...
# Build
cd packages/killer-core && npx tsc
cd packages/killer-app && pnpm run build

# Start (demo mode, no API key needed)
node packages/killer-app/dist/main.js

# Start with LLM
KILLER_LLM_PROVIDER=anthropic ANTHROPIC_API_KEY=sk-... node packages/killer-app/dist/main.js

# Start with API server
node packages/killer-app/dist/main.js --api --port 3000

# Help
node packages/killer-app/dist/main.js --help
```
...
2. **Environment variables** — `KILLER_LLM_PROVIDER`, `KILLER_API_KEY`, `KILLER_MODEL`
3. **Project config** — `.killer/config.json`
4. **User config** — `~/.killer/config.json`
5. **Defaults**
...
|----------|---------|---------|
| `KILLER_LLM_PROVIDER` | `anthropic`, `openai`, `openrouter`, `mock` | `anthropic` |
...
| `KILLER_API_TOKEN` | Bearer token for API auth | none |
| `KILLER_LOG_LEVEL` | `debug`, `info`, `warn`, `error`, `silent` | `info` |
...
Start with `--api` flag. All endpoints accept `Authorization: Bearer <token>` if `KILLER_API_TOKEN` is set.
...
# Run tests
cd packages/killer-core && npx vitest run   # 245 tests
cd packages/killer-app && npx vitest run     # 1119 tests
...
- **`packages/killer-core`** — Kernel with zero external dependencies. Brain systems, memory, evolution, synapse protocol.
- **`packages/killer-app`** — Application layer. Orchestrator, LLM providers, persona engine, CLI, API server, plugins.
