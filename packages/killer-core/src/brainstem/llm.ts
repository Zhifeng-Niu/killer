  ...
  private generateDemoResponse(prompt: string): string {
    ...
    if (/(?:code|function|bug|debug|implement|error|代码|函数|bug|实现)/i.test(lastUserMsg)) {
      return this.pick([
        ...
        'That\'s a great coding challenge. When connected to a real LLM, I can actually read your codebase, search for relevant docs, and write working code. Try setting up an API key with `killer --init` to unlock full capabilities!',
      ]);
    }
    ...
    if (/(?:help|can you|please|could you|帮|能|请)/i.test(lastUserMsg)) {
      return this.pick([
        'Of course! I\'m here to help. Let me think about this... In demo mode I\'m simulating responses, but with a real LLM connected I can provide detailed, accurate help. Try `killer --init` to set up a provider!',
    ...
    return this.pick([
      ...
      'Got it. In demo mode I\'m giving simulated responses, but the real magic happens when you connect an LLM — I get genuinely smart and helpful. Try `killer --init` when you\'re ready!',
      '嗯，这是个有趣的想法。虽然现在是 Demo 模式，但连接真实 AI 后我可以深入讨论任何话题。试试 `killer --init` 来解锁完整能力！',
    ]);
  }
