...
export class EvolutionEngine {
   ...
   * 达尔文演化主循环
   */
  evolve(population: CellDNA[]): EvolutionCandidate[] {
   ...
   * 变异：随机修改 DNA 子集
   */
  private mutate(dna: CellDNA): CellDNA {
   ...
   * 交叉：两个 DNA 交换片段
   */
  private crossover(a: CellDNA, b: CellDNA): CellDNA {
    const child: CellDNA = {
   ...
   * 选择：适应度评估
   */
  select(candidates: EvolutionCandidate[], fitnessScores: FitnessScore[]): CellDNA[] {
    // 按适应度排序
   ...
   * 评估适应度
   */
  evaluateFitness(dna: CellDNA, history: FitnessReflectionOutcome[]): FitnessScore {
    if (history.length === 0) {
      return {
        overall: 0.5,
        taskSuccess: 0.5,
