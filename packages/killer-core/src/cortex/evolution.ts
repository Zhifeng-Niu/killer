...
export interface IEvolutionEngine {
   ...
   * 生成变异候选
   */
  generateMutations(dna: CellDNA, count: number): Promise<Mutation[]>;

  /**
   * 应用变异到 DNA
   */
  applyMutation(dna: CellDNA, mutation: Mutation): Promise<CellDNA>;

  /**
   * DNA 交叉重组
   */
  crossover(dna1: CellDNA, dna2: CellDNA): Promise<CellDNA>;
   ...
   * 获取 Cell 种群
   */
  getPopulation(): Promise<CellDNA[]>;

  /**
   * 创建子 Cell
   */
  fission(parentDna: CellDNA): Promise<CellDNA>;

  /**
   * 融合两个 Cell
   */
  fusion(dna1: CellDNA, dna2: CellDNA): Promise<CellDNA>;
